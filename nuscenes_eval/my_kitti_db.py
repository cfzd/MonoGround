...
class MyKittiDB:
    ...
    def standardize_sample_token(token: str) -> Tuple[str, str]:
        ...
        :param token: MyKittiDB unique id.
    ...
    def parse_label_line(label_line) -> dict:
        ...
        :param label_line: Single line from MyKittiDB label file.
    ...
    def get_filepath(token: str, table: str, root: str='/data/sets/kitti') -> str:
        ...
        """
        folder, filename = MyKittiDB.standardize_sample_token(token)
    ...
    def get_transforms(token: str, root: str='/data/sets/kitti') -> dict:
        ...
        """
        calib_filename = MyKittiDB.get_filepath(token, 'calib', root=root)
    ...
    def get_pointcloud(token: str, root: str = '/data/sets/kitti') -> LidarPointCloud:
        ...
        """
        pc_filename = MyKittiDB.get_filepath(token, 'velodyne', root=root)
    ...
    def get_boxes(self,
        ...
        with open(MyKittiDB.get_filepath(token, 'label_2', root=self.pred_root), 'r') as f:
            for line in f:
    ...
    def get_boxes_2d(self,
        ...
        boxes = []
        names = []
        with open(MyKittiDB.get_filepath(token, 'label_2', root=self.root), 'r') as f:
            for line in f:
                # Parse this line into box information.
                parsed_line = self.parse_label_line(line)
    ...
    def render_sample_data(self,
        ...
        if sensor_modality == 'lidar':
            ...
            im_path = MyKittiDB.get_filepath(token, 'image_2', root=self.root)
