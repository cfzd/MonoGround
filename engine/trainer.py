...
def do_eval_nuscenes(cfg, model, data_loaders_val, iteration,):

	from nuscenes import NuScenes
	from nuscenes.eval.common.config import config_factory
	from nuscenes.eval.detection.evaluate import DetectionEval
	from scripts.nuscenes2kitti import KittiConverter
	cfg = config_factory('detection_cvpr_2019')
	nusc = NuScenes(version='v1.0-trainval', dataroot='/home/zequn/datasets/nuscenes', verbose=False)
	...
	nusc_eval = DetectionEval(nusc, cfg, self.res_mockup, eval_set='val', output_dir=nuscenes_predict_folder,
								verbose=False)

def do_eval(cfg, model, data_loaders_val, iteration, nuscenes_category = False):
...
def do_test(cfg, model, data_loaders_val, iteration, nuscenes_category = False):
	eval_types = ("detection",)
	dataset_name = cfg.DATASETS.TEST[0]
...
def do_train(
	...
	for data, iteration in zip(data_loader, range(start_iter, max_iter)):
		...
		if iteration % 20 == 0 and comm.get_rank() == 0:
		...
		if iteration % cfg.SOLVER.SAVE_CHECKPOINT_INTERVAL == 0:
			...
			if comm.get_rank() == 0:
				if iteration // arguments["iter_per_epoch"] < 80:
					checkpointer.save("model_checkpoint", **arguments)
				else:
					checkpointer.save("model_checkpoint_"+str(int(iteration // arguments["iter_per_epoch"])), **arguments)
		...
		if iteration % cfg.SOLVER.EVAL_INTERVAL == 0:
			...
			result_dict, result_str, dis_ious = do_eval(cfg, model, data_loaders_val, iteration, nuscenes_category = cfg.DATASETS.NUSCENES_CATEGORY)
			...
			if comm.get_rank() == 0:
				...
				if cfg.DATASETS.NUSCENES_CATEGORY:
					important_key = 'mAP'
					for key, value in result_dict.items():
						writer.add_scalar('nuscenes_eval/'+key, value, cur_epoch)
