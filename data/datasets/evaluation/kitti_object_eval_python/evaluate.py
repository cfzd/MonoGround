...
def _read_imageset_file_nuscenes(path):
    with open(path, 'r') as f:
        lines = f.readlines()
...
def evaluate(label_path,
    ...
    if not nuscenes_category:
        ...
        dt_annos = kitti.get_label_annos_nuscenes(result_path)
    ...
    if not nuscenes_category:
        ...
        val_image_ids = _read_imageset_file_nuscenes(label_split_file)
        gt_annos = kitti.get_label_annos_nuscenes(label_path, val_image_ids)

    if coco:
        return get_coco_eval_result(gt_annos, dt_annos, current_class)
    else:
        return get_official_eval_result(gt_annos, dt_annos, current_class, metric=metric, nuscenes_category=nuscenes_category)

def generate_kitti_3d_detection(prediction, predict_txt, nuscenes_category = False):
