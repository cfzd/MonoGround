...
def train(cfg, model, device, distributed):
    ...
    if len(cfg.MODEL.WEIGHT) > 0:
        ...
        if cfg.MODEL.HEAD.LOAD_MODEL_META:
...
def main(args):
    ...
    if args.eval_only:
        ...
        return run_test(cfg, checkpointer.model, vis=args.vis, eval_score_iou=args.eval_score_iou, eval_all_depths=args.eval_all_depths, nuscenes_category = cfg.DATASETS.NUSCENES_CATEGORY)
    ...
    if args.test:
        ...
        return run_test(cfg, checkpointer.model, vis=args.vis, eval_score_iou=args.eval_score_iou, eval_all_depths=args.eval_all_depths, vis_test = True, nuscenes_category = cfg.DATASETS.NUSCENES_CATEGORY)
